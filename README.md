...
`echo "Новая строка" > <file>` - если `<file>` пустой - запишет "Новая строка"; если `<file>` заполнен - сотрёт все записи и запишет "Новая строка".

`echo "Новая строка" >> <file>` - "Новая строка" будет записано на второй строке.
